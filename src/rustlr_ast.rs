...
pub enum TypeList<'lt> {
  TypeList(LBox<Type<'lt>>,Vec<LC<Type<'lt>>>),
  TypeList_Nothing,
}
...
pub enum Expr<'lt> {
  ...
  If{_item0_:&'lt str,cond:LBox<Expr<'lt>>,_item2_:&'lt str,THEN:LBox<Expr<'lt>>,_item4_:LBox<Else<'lt>>},
  Expr_Nothing,
}
...
pub enum Else<'lt> {
  Unit,
  Else{_item0_:&'lt str,els:LBox<Expr<'lt>>},
  Else_Nothing,
}
...
#[derive(Debug)]
pub enum FunType<'lt> {
  FunMulti{ty:LBox<TypeList<'lt>>,ret:LBox<Type<'lt>>},
  FunSingle{ty:LBox<Type<'lt>>,ret:LBox<Type<'lt>>},
  FunType_Nothing,
}
