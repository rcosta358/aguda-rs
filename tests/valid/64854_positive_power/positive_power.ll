; ModuleID = './tests/valid/64854_positive_power/positive_power.agu'
source_filename = "./tests/valid/64854_positive_power/positive_power.agu"
...
define i32 @positivePower(i32 %0, i32 %1) {
entry:
  ...
  %leq = icmp sle i32 %k1, 0
  %if_cond = icmp ne i1 %leq, false
...
then:                                             ; preds = %entry
  ...
  %leq6 = icmp sle i32 %p5, 0
  %if_cond7 = icmp ne i1 %leq6, false
  br i1 %if_cond7, label %then2, label %else3

merge:                                            ; preds = %merge4, %then
  %phi10 = phi i32 [ 1, %then ], [ %phi, %merge4 ]
  ret i32 %phi10
...
else3:                                            ; preds = %else
  ...
  %pow = call i32 @__pow__(i32 %p8, i32 %k9)
  br label %merge4

merge4:                                           ; preds = %else3, %then2
  %phi = phi i32 [ 0, %then2 ], [ %pow, %else3 ]
  br label %merge
}

define i32 @main({} %0) {
entry:
  %call = call i32 @positivePower(i32 2, i32 3)
  %call1 = call i32 @positivePower(i32 2, i32 3)
