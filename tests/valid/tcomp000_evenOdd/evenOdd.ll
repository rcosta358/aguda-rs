; ModuleID = './tests/valid/tcomp000_evenOdd/evenOdd.agu'
source_filename = "./tests/valid/tcomp000_evenOdd/evenOdd.agu"
...
entry:
  ...
  %eq = icmp eq i32 %x1, 0
  %not = xor i1 %eq, true
  br i1 %not, label %rhs, label %merge
...
merge:                                            ; preds = %rhs, %entry
  %logical_op = phi i1 [ %eq, %entry ], [ %call, %rhs ]
  ret i1 %logical_op
}
...
entry:
  ...
  store i32 %0, ptr %x, align 4
  %x1 = load i32, ptr %x, align 4
  %neq = icmp ne i32 %x1, 0
  br i1 %neq, label %rhs, label %merge
...
merge:                                            ; preds = %rhs, %entry
  %logical_op = phi i1 [ %neq, %entry ], [ %call, %rhs ]
  ret i1 %logical_op
}

define i32 @main({} %0) {
entry:
  %call = call i1 @odd(i32 17)
  %call1 = call i1 @odd(i32 17)
