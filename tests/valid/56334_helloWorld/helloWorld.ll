; ModuleID = './tests/valid/56334_helloWorld/helloWorld.agu'
source_filename = "./tests/valid/56334_helloWorld/helloWorld.agu"
...
cond:                                             ; preds = %merge, %entry
  ...
  %geq = icmp sge i32 %i1, 0
  %while_cond = icmp ne i1 %geq, false
...
then:                                             ; preds = %body
  call void @__print_int__(i32 55)
  br label %merge

else:                                             ; preds = %body
  %i6 = load i32, ptr %i, align 4
  %eq7 = icmp eq i32 %i6, 1
  %if_cond8 = icmp ne i1 %eq7, false
  br i1 %if_cond8, label %then3, label %else4

merge:                                            ; preds = %merge5, %then
  %phi9 = phi {} [ zeroinitializer, %then ], [ %phi, %merge5 ]
...
then3:                                            ; preds = %else
  call void @__print_int__(i32 77)
  br label %merge5

else4:                                            ; preds = %else
  call void @__print_int__(i32 99)
  br label %merge5

merge5:                                           ; preds = %else4, %then3
  %phi = phi {} [ zeroinitializer, %then3 ], [ zeroinitializer, %else4 ]
  br label %merge
}

define i32 @main({} %0) {
entry:
  call void @helloWorld({} zeroinitializer)
  ret i32 0
}
