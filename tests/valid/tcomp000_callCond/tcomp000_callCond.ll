; ModuleID = './tests/valid/tcomp000_callCond/tcomp000_callCond.agu'
source_filename = "./tests/valid/tcomp000_callCond/tcomp000_callCond.agu"
...
merge3:                                           ; preds = %else2, %merge6
  %phi8 = phi i32 [ %phi7, %merge6 ], [ 7, %else2 ]
  %call = call i32 @add(i32 %phi, i32 %phi8)
...
merge11:                                          ; preds = %else10, %then9
  %phi12 = phi i32 [ 3, %then9 ], [ 4, %else10 ]
  br i1 true, label %then13, label %else14

then13:                                           ; preds = %merge11
  br i1 false, label %then16, label %else17

else14:                                           ; preds = %merge11
  br label %merge15

merge15:                                          ; preds = %else14, %merge18
  %phi20 = phi i32 [ %phi19, %merge18 ], [ 7, %else14 ]
  %call21 = call i32 @add(i32 %phi12, i32 %phi20)
  call void @__print_int__(i32 %call21)
  ret i32 0
...
merge18:                                          ; preds = %else17, %then16
  %phi19 = phi i32 [ 5, %then16 ], [ 6, %else17 ]
  br label %merge15
}
