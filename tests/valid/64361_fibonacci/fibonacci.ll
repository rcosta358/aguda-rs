; ModuleID = './tests/valid/64361_fibonacci/fibonacci.agu'
source_filename = "./tests/valid/64361_fibonacci/fibonacci.agu"
...
then:                                             ; preds = %entry
  ...
  %eq6 = icmp eq i32 %n5, 1
  %if_cond7 = icmp ne i1 %eq6, false
  br i1 %if_cond7, label %then2, label %else3

merge:                                            ; preds = %merge4, %then
  %phi12 = phi i32 [ 0, %then ], [ %phi, %merge4 ]
  ret i32 %phi12
...
else3:                                            ; preds = %else
  ...
  %sub10 = sub i32 %n9, 2
  %call11 = call i32 @fib(i32 %sub10)
  %add = add i32 %call, %call11
  br label %merge4

merge4:                                           ; preds = %else3, %then2
  %phi = phi i32 [ 1, %then2 ], [ %add, %else3 ]
  br label %merge
}
...
entry:
  ...
  %call2 = call i32 @fib(i32 1)
  %call3 = call i32 @fib(i32 1)
  call void @__print_int__(i32 %call3)
  %call4 = call i32 @fib(i32 5)
  %call5 = call i32 @fib(i32 5)
  call void @__print_int__(i32 %call5)
  %call6 = call i32 @fib(i32 10)
  %call7 = call i32 @fib(i32 10)
