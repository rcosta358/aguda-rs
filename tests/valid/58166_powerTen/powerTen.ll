; ModuleID = './tests/valid/58166_powerTen/powerTen.agu'
source_filename = "./tests/valid/58166_powerTen/powerTen.agu"
...
then:                                             ; preds = %entry
  ...
  %eq = icmp eq i32 %n5, 0
  %if_cond6 = icmp ne i1 %eq, false
  br i1 %if_cond6, label %then2, label %else3

merge:                                            ; preds = %merge4, %then
  %phi8 = phi {} [ zeroinitializer, %then ], [ %phi, %merge4 ]
...
else3:                                            ; preds = %else
  ...
  %sub = sub i32 %n7, 1
  %call = call i32 @powerTen(i32 %sub)
  %mul = mul i32 10, %call
  store i32 %mul, ptr %result, align 4
  br label %merge4

merge4:                                           ; preds = %else3, %then2
  %phi = phi {} [ zeroinitializer, %then2 ], [ zeroinitializer, %else3 ]
  br label %merge
}

define i32 @main({} %0) {
entry:
  %call = call i32 @powerTen(i32 7)
  %call1 = call i32 @powerTen(i32 7)
